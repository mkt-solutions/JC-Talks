...
export default function App() {
  ...
  useEffect(() => {
    if (sessionDuration >= 3600 && !hasSentBibleSuggestion && user && messages.length > 0) {
      setHasSentBibleSuggestion(true);
      setIsTyping(true);
      getBibleSuggestion(messages, user)
        .then(response => {
          setMessages(prev => [...prev, { role: 'assistant', content: response }]);
        })
        .finally(() => setIsTyping(false));
    }
  }, [sessionDuration, hasSentBibleSuggestion, user, messages]);
    ...
    <div className="min-h-screen bg-[#F5F2ED] flex flex-col font-serif">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-[#5A5A40]/10 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-1 bg-[#5A5A40]/5 rounded-full text-[10px] font-medium text-[#5A5A40]/60">
            <Clock className="w-3 h-3" />
            {t('header.sessionTime', { time: formatDuration(sessionDuration) })}
          </div>
          {!user.is_subscribed && (
