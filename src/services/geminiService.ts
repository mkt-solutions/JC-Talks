...
export async function getBibleSuggestion(messages: Message[], user: User) {
  const age = calculateAge(user.dob);
  const lang = LANGUAGES[user.language as Language] || user.language;
  ...
  const model = ai.models.generateContent({
    ...
    contents,
    config: {
      systemInstruction: `You are an artificial intelligence inspired by the teachings of Jesus Christ. 
      The user ${user.name} has been talking to you for over an hour now. 
      Based on the conversation history, identify the "pain" or the main struggle they are facing right now.
      Provide a warm, compassionate suggestion to read the Bible. 
      Explain WHY you are suggesting the Bible specifically in relation to the pain they are feeling. 
      Be gentle, fatherly, and deeply spiritual. 
  ...
  const response = await model;
  return response.text || "My child, I suggest you seek comfort in the Holy Scriptures, for they hold the words of life you need right now.";
}
